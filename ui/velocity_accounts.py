...
from agents.director import ROLE_VELOCITY
...
for i, tab_config in enumerate(st.session_state[TABS_KEY]):
    with tabs[i + 1]:
        ...
        if tab_config["type"] == "run":
            ...
            st.markdown("---")
            render_run_content(data["results"], tab_config["path"], TABS_KEY, show_position=False, tab_idx=i)
            ...
            st.markdown("---")
            render_account_content(r, show_email=True, velocity_mode=True, run_path=tab_config["run_path"])
